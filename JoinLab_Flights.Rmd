...
```{r}
airports
left_join(avg_arrival_delay, airports, join_by(dest == faa))
```
...
```{r}
airports
avg_arrival_delay %>% arrange(-avg_arriv_delay) %>% slice_max(avg_arriv_delay)

#or

avg_arrival_delay %>% slice_max(avg_arriv_delay)
...
my_flights
airlines
avg_dep_del <- my_flights %>% group_by(carrier) %>% summarise(avg_del = mean(dep_delay)) %>% left_join(airlines)
avg_dep_del
```

Which airline had the smallest average departure delay? 
```{r}
avg_dep_del %>% slice_min(avg_del)
```
